...
Import of and throwError from 'rxjs'.`;
  ...
  const writeTestPromptOptions = [
    `Write a unit test for the following component.`
  ];
...
${form.getFieldValue('code')}`;
    const writeTestPrompt = form.getFieldValue('writeTestPrompt');
  ...
  return (
    <>
      ...
      >
        ...
        <Form.Item
          label="Write Test Prompt"
          name="writeTestPrompt"
        >
          <Select
            allowClear={true}
            options={writeTestPromptOptions.map(_item => ({label: _item, value: _item}))}
          />
        </Form.Item>
